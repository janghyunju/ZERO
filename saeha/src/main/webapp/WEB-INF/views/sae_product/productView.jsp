...
<body>
			...
			<c:forEach items="${replyList}" var="replyList">
				...
				<div>
					...
					<c:if test="${member.userId == 'admin' }">
						...
						<button type="button">
							<a href="/sae_product/pdreplywriteView">답변달기</a>
						</button>
					</c:if>
				</div>
	</div>
	...
	<div>
	<c:forEach items="${pdreply}" var="pdreply">
	<p>
	작성자: ${pdreply.py_writer}님	
	<fmt:formatDate value="${pdreply.py_date}" pattern="yyyy-MM-dd"/>
	</p>
	<p>A : ${pdreply.py_content}</p>
	</c:forEach>
	</div>
	</c:if>
